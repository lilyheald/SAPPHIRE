...
long_data$Ethnicity[long_data$Ethnicity == 'Cape_colored'] <- 'Cape_mixed'
long_data
```
...
combine_plots(
  ...
  plotgrid.args = list(nrow = 2),
  annotation.args = list(title = "Effect of season on CIElab metrics across ethnicity",
                theme = ggplot2::theme(plot.title = element_text(size = 10)))
)

combine_plots(
  plotlist = list(c1, c2, c4, c6),
  plotgrid.args = list(nrow = 1),
  annotation.args = list(title = "Significant effects of season on CIElab metrics across ethnicity",
                theme = ggplot2::theme(plot.title = element_text(size = 15)))
)
```
...
```{r fig.width = 10}
cm <- long_data[long_data$Ethnicity == "Cape_mixed",]
cm1 <- ggwithinstats(
  data = xh,
  x = Season,
  y = InnerArmM,
...
combine_plots(
  plotlist = list(cm1, cm2, cm3, cm4, cm5, cm6, cm7, cm8),
  plotgrid.args = list(nrow = 4),
  annotation.args = list(
    title = "Effect of season on xh color metrics",
    theme = ggplot2::theme(plot.title = element_text(size = 15)))
)
```
...
```{r cm}
xh <- long_data[long_data$Ethnicity == "Xhosa",]
```
