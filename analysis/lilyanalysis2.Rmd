...
full_IDs <- seq(min(pheno_dat$IID), max(pheno_dat$IID))
complete_pheno_dat <- full_IDs %>%
  left_join(full_IDs, pheno_dat, by = "IID")

complete_data <- full_IDs %>%
  data.frame(ID = .) %>%
  left_join(data, by = "ID")

write.table(pheno_dat, "~/sapphire/SAfrADMIX/pheno_dat.txt", sep = " ", dec = ".", row.names = F)
write.table(tanning_data, "~/sapphire/SAfrADMIX/tanning_data.txt", sep = " ", dec = ".", row.names = F)
mdiff_effect <- pheno_dat %>%
  select(matches("FID|IID|tanning"))
write.table(mdiff_effect, "~/sapphire/SAfrADMIX/mdiff_effect.txt", sep = " ", row.names = F)

missingids <- anti_join(tanning_data, SAfrADMIX, by = c("IID" = "V2"))[,c("FID","IID")]

write.table(missingids, "~/sapphire/SAfrADMIX/remove.txt", sep = " ", dec = ".", row.names = F, col.names = c("FID","IID"))
```
...
ggqqplot(cape_colored, "MedianInnerArmM", facet.by = "Season")
ggqqplot(xhosa, "MedianInnerArmM", facet.by = "Season")

aov_result_x <- aov(MedianInnerArmM ~ Season, data = xhosa)
